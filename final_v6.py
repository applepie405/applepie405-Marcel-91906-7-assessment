...
def update_balance():
  ...
  # Append each comicss balance, progress and goal to the label
  for comic in comic_list:
...
def sell_comic(comic):
  if comic.sell(amount.get()):
    ...
    action_feedback.set("Great! Total of ${:.2f} sold into {}".format(amount.get(), comic.name))
...
happy_image = PhotoImage(file="book.png")
sad_image = PhotoImage(file="book.png")
